...
STA 0028h
Lxi h,klava
SHLD 0029h; надо оформить переход к ПП анализа клавиатуры по прерываниям, для этого по адресу 0028h
...
; чтобы появился строб клавы надо возбудить линии сканирования в выделенном фрагменте
mvi a,03h
sta 8003h
...
LDA 8008h ; читаем региср возврата
...
sta 8003h; сброс регистра возврата 
mvi a,03h
sta 8003h ; возбуждаем 3-ю линию
LDA 8004h ; читаем региср возврата
ani 03h ; выделяем код возврата(это на всякий случай)
cpi 0h
jnz L3 ;если не 0- одна из клавишь на 2-й линии нажата
: L1
cpi 01h
jz K11  ;  если не 0
